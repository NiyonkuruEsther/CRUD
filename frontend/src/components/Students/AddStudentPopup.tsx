...
const AddStudentPopup = ({isOpen, onClose, onAddStudent, role}: AddStudentPopupProp) => {
  ...
  const handleImageUpload = (event) => {
    ...
    // Read the image file and generate the URL
    const reader = new FileReader();
    reader.onload = () => {
      const imageUrl = reader.result;

      // Pass the generated URL to the backend for further processing
      setUrl(imageUrl);
    };
      ...
      <div className="bg-white p-6 rounded-md shadow-lg">
        ...
        <form onSubmit={handleSubmit} >
          ...
          <form className="border p-4 my-4">
               ...
               { url ? (<label htmlFor="file-upload" className="flex flex-col items-center justify-center gap-2 w-full h-full cursor-pointer">
<BiUpload size={60} className='text-orange-500'/>                    
<div>
                        <p className="text-base text-center text-green-600">
                            <span className='leading-5 font-semibold text-black'>Image uploaded  {" "} </span>
                            successfully</p>
                    </div>
                </label>)  : (<label htmlFor="file-upload" className="flex flex-col items-center justify-center gap-2 w-full h-full cursor-pointer">
<BiUpload size={60} className='text-orange-500'/>                    
<div>
