...
const UpdateProfileInformationForm = () => {
    ...
    useEffect(() => {
        if (user !== undefined) {
            setName({fname: user.fname, mname: user.mname, lname: user.lname})
            setEmail(user.email)
        }
    }, [user])
                ...
                user ?. must_verify_email && user ?. email_verified_at === null && (
                    <div>
                        <p className="text-sm mt-2 text-gray-800 dark:text-gray-200">
                            ...
                            <button className="underline text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800"
                                onClick={
